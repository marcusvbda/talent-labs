...
2. **Preferences are a saved profile, not a table filter.** The client
   ...
   filters in the listing itself. With no filter saved, the listing is the
   whole eligible pool, unfiltered (B.3.2).
...
- **Sending is always manual (owner decision, after Phase 18).** There is
  no automatic sending: the system never sends an application on its own.
  The client picks a job in the Jobs list and clicks "Apply". The
  dispatcher command, its schedule and the "Automatic sending" toggle were
  removed.
- **The client can only apply through our interface.** Nothing in `/app`
  (UI or data sent to the browser) may let the client leave the platform to
  apply on their own: no link to the original posting, no apply URL, no
  company email address. Text shown to the client (posting descriptions,
  rendered email previews, the subject/body of sent applications) has links
  and email addresses replaced by `[link]` / `[email]`
  (`ClientSafeText`); the emails actually sent and the stored `applications`
  rows are never redacted. `/admin` is unaffected. This replaces the
  earlier rule that contact emails were visible to the client for their own
  sent applications, and keeps `contact-discovery-mvp` B.10's "never
  outside /admin" rule intact.
...
- **Sender reputation is the client's.** Emails leave from the client's
  personal Gmail. The 10/day cap (B.7; no spacing between sends) keeps it well under
...
collection run ─► posting inserted ─► DiscoverContactsForPosting (existing)
                 ...
                 posting now visible to every client whose preferences match
                                              │
                          manual "Apply" (client, in the Jobs list)
                                              │
                                              ▼
...
- `composer dev` runs **one independent queue worker per queue** —
  `collection`, `contacts`, `ai`, `outreach` and `default`, each its own
  process — so a slow queue (contact discovery probes SMTP) never delays
  another (an application must be sent as soon as it is queued). There is no
  scheduler: nothing runs on a schedule in this feature.
...
**Single source of truth** — the client listing and the batch "Send to
selected" action both use this exact query. A posting matches when:

- its company has `outreach_status = verified`, **and**
- it has a profile with `status = done`, **and**
- its company has a recipient we can write to: an `smtp_verified` contact
  whose `local_part` is in `RECIPIENT_PRIORITY` (B.7.3), **and**
- this client has **no** application yet for that company (a company
  already contacted, queued or failed, leaves the client's list), **and**
- every preference field the client **filled in** is satisfied (empty
  ...
  are ORed):
    ...
    - **stack** — profile `stack` shares at least one item with the client's
      stack (`jsonb ?| array[...]`);
    - **location** — filled only when the client has at least one location:
      then (`accepts_remote` and `profile.is_remote = true`) **or** any
      client location appears (`ILIKE`) in `job_postings.location` or in any
      `profile.locations` item. With no locations there is no location
      restriction, whatever `accepts_remote` says: the toggle only _widens_
      a filled location list, it never restricts on its own.

The first four conditions define the **eligible pool**: every posting this
client can still send an application for. If the client has saved **no**
preference at all, or every field is empty, the query returns the whole
eligible pool, unfiltered; each field the client fills in then narrows it.
A client with no filters can therefore send to any job of the pool (still
...
final class OutreachLimits
{
    ...
    public const MAX_POSTING_AGE_DAYS = 14;      // reserved (was used by the removed auto-send)
...
#### B.7.6 Dispatcher — removed

There is no automatic dispatcher (owner decision, after Phase 18): no
`applications:dispatch` command, no schedule, no `schedule:work`. Every
application starts from the client's manual "Send to selected". There is
**no spacing** between sends (owner decision): `QueueApplication` inserts the
row and dispatches `SendApplicationEmail` immediately (no delay); the
`outreach` worker sends as fast as it picks the jobs up. `scheduled_for` is
kept only as the moment the send was requested.

**Test mode:** when `OUTREACH_INTERCEPT_TO` is set in `.env`, every email is
delivered to that address instead of the company (subject prefixed with
`[TEST — would go to <real recipient>]`, header `X-TalentLabs-Intercepted`),
while the `applications` row still records the real recipient. Empty or
unset means emails go to the companies for real. An invalid address never
falls back to the real recipient: nothing is sent.
...
1. **Preferences page** (custom Filament page, one form, "Save" button):
    ...
    - No "Automatic sending" section: sending is always manual.
2. **Jobs page** (replaces today's-jobs list): `MatchingJobPostings::forUser`,
   25/page, newest `published_at` first. The list only contains jobs the
   client can still send to (B.3.2): companies without a usable recipient
   and companies already contacted do not appear. The table is deliberately
   lean — columns: title, company, location/remote. Everything else
   (department, employment type, published, stack, summary, description)
   lives only in the row's **View** modal, with links and emails in the
   description redacted. There is no "Open posting" action, no source label
   and **no per-row send button**. Sending is a **batch**: a select column
   (checkboxes) and a bulk action **"Send to selected"** that opens a
   confirmation modal (selected jobs, distinct companies, company names —
   never recipient addresses —, attachment name, "N of LIMIT applications
   left today") and then calls `QueueApplication` (`origin = manual`) for
   one posting per selected company. **Daily limit:** the action is
   disabled (with the reason as tooltip) when the client isn't eligible or
   the limit is exhausted, and a selection with more companies than the
   remaining quota is refused as a whole — nothing is sent — with a message
   asking to deselect some jobs. The table also caps the selection at the
   remaining quota (`maxSelectableRecords`; Filament truncates an oversized
   selection server-side to that many rows, so the daily limit is never
   exceeded). The sends of a batch are dispatched immediately. Empty state: "No matching jobs yet." (an
   empty list means no eligible posting satisfies the saved filters, or
   none has been analysed yet).
3. **Applications page** (read-only table of the client's own
   `applications`): company, job title, status badge, origin, queued/sent
   at (no recipient address); view modal shows the subject/body sent, with
   links and emails redacted for display. Live-updating via the realtime
   driver (event payload carries only ids, never email content).
...
- **AC06** — The Jobs page shows only postings satisfying B.3.2 for the
  ...
  lists; no filters saved → the whole eligible pool, unfiltered; each
  filled field narrows the list.
...
- **AC10** — A second application to the same company by the same client
  is impossible (the company leaves the client's Jobs list,
  `QueueApplication` returns null, and a direct insert fails on the unique
  index).
- **AC11** — The 11th application of the day for a client is refused
  (every application is manual; a batch larger than the remaining quota is
  refused as a whole and the bulk action is disabled once the limit is
  reached); changing `DAILY_SEND_LIMIT` changes the behavior with
  no other code change.
- **AC12** — _(removed: there is no automatic sending.)_ Nothing in the
  system sends an application unless the client clicked "Apply"; no
  scheduled task exists (`php artisan schedule:list` is empty).
...
4. Owner-assisted end-to-end: owner uses "Apply"
...
7. Report `composer dev` includes the queues listed in B.2.6 (no
   `schedule:work`: nothing is scheduled).
